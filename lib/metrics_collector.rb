...
class MetricsCollector
  ...
  def run(inventoried_time, morefs_to_meter=[])
    ...
    machine_morefs.each_slice(configuration[:vsphere_readings_batch_size]).each do |morefs|
      ...
      results.each do |vm, result|
        ...
        reading = Reading.build_from_result(result, @local_inventory[vm.moref])
        if ( reading.reading_at )
          collected_time= reading.reading_at # Save this for faked readings below (in case it somehow inexplicably doesn't match time_to_query)
        end
    ...
    @readings.each{|r| r.reading_at ||= collected_time }
  ...
  def self.level_3_statistics_enabled?
    ...
    VSphere::wrapped_vsphere_request {
  ...
  def filtered_inventory_morefs
    @local_inventory.select{|platform_id, machine|
      machine.record_status != 'incomplete' }.keys
  end
  ...
  def custom_retrieve_stats(objects, metrics, opts = {})
    ...
    metrics.each do |metric|
      ...
      if !counter
        ...
        fail "Counter for #{metric} couldn't be found"
      end

      metric_ids << RbVmomi::VIM::PerfMetricId(:counterId => counter.key,
                                               :instance => metric.match(/^(?:cpu|mem)/) ? '' : '*')  #!! refactor Reading.machine_properties et al

    end
    query_specs = objects.map {|obj|
