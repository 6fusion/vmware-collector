...
class InventoryCollector
  ...
  def initialize(infrastructure)
    ...
    VSphere::wrapped_vsphere_request { @vsphere_session.propertyCollector.CreateFilter(spec: InventoryCollector::vm_filter_spec(
                                                                                         InventoryCollector::vm_properties, @data_center.vmFolder),
                                                                                       partialUpdates: false) }
    # These two maps are needed to get cpu_speed_mhz from Hosts for Machines
    @hosts_cpu_hz_map = Hash.new
    @vm_hosts_map = Hash.new
  ...
  def run(time_to_query)
    ...
    results = VSphere::wrapped_vsphere_request { @vsphere_session.propertyCollector.WaitForUpdatesEx(version: @version,
                                                                                                     options: {maxWaitSeconds: 0}) }
    collected_machines = Array.new
        ...
        fs.objectSet.each do |os|
          begin
            ...
            if (os.obj.is_a?(RbVmomi::VIM::VirtualMachine))
              ...
              if ( runtime_host and runtime_host.val )
              ...
              #machine.cpu_speed_mhz = @infrastructure.cpu_speed_for(machine.platform_id) / 1_000_000.0
              ...
              if ( machine.record_status == 'incomplete')
                if ( @local_inventory.has_key?(machine.platform_id) )
                ...
                #  (The events generated by a migration don't easily allow us to actually determine if that is what is going on)
                  ...
                  unless ( machine.name.blank? or machine.platform_id.blank? )
